...
export default function AdminMain() {
  ...
  const delUser = async (id) => {
    try {
      ...
      if (response.ok) {
        setUsers((pre) => (pre.filter((el) => el.id !== id)));
      } else {
        throw new Error();
      }
    } catch (error) {
  ...
  return (
    <div className={styles.mainList}>
      <table className={styles.table}>
        <thead>
          <tr>
            <th className={styles.th}>Имя/ Login</th>
            <th className={styles.th} colSpan="5">Программа тренировок</th>
          </tr>
        </thead>
        <tbody>
          {users.map((el) => (
            <tr>
              ...
              <td className={styles.td}>
                <button type="button" className={styles.button}>
                  <Link className={styles.adminlink} to={`parametrs/${el.id}`}>
              ...
              <td className={styles.td}>
                <button type="button" className={styles.button}>
                  <Link className={styles.adminlink} to={`statistic/${el.id}`}>
              ...
              <td className={styles.td}>
                <button type="button" className={styles.button}>
                  <Link className={styles.adminlink} to={`wschat/${el.id}`}>
              ...
              <td className={styles.td}>
                <button type="button" className={styles.button}>
                  <Link className={styles.adminlink} to={`/admin/training/${el.id}`}>
              ...
              <td className={styles.td}>
                <button type="button" className={styles.button} onClick={() => delUser(el.id)}>
